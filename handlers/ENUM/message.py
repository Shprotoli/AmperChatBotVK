...
class EQuit(Enum):
    SUCCESS = "👀 @id{user_id} (Пользователь) покидает эту вечеринку"
    SELF_ADMIN = "⛔ Я не могу кикнуть @id{user_id} (тебя), потому что ты являешься администратором данной беседы"

class ERandom(Enum):
    SUCCESS = "🤖 @id{user_id} (Пользователю) выпало число: {value_random}"
    INCORRECT_FORMAT = "⚠ @id{user_id} (Вы) должны передать два числовых параметра в порядке возрастания\n❓ Например: /random 0 10"