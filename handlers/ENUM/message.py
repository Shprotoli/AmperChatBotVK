...
class EQuit(Enum):
    SUCCESS = "👀 @id{id_user} (Пользователь) покидает эту вечеринку"
    SELF_ADMIN = "⛔ Я не могу кикнуть @id{id_user} (тебя), потому что ты являешься администратором данной беседы"